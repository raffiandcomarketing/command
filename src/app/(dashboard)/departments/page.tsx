...
export default function DepartmentsPage() {
  ...
  const DepartmentCard = ({ dept }: { dept: (typeof departments)[0] }) => {
    ...
    return (
      <Card className="bg-white border border-stone-200 rounded-xl shadow-sm hover:shadow-md hover:border-[#09203F]/20 transition-all group cursor-pointer">
        <div className="p-6 h-full flex flex-col">
          ...
          <h3 className="text-lg font-semibold text-stone-900 mb-2 group-hover:text-[#09203F] transition-colors">
            {dept.name}
          </h3>
          <p className="text-sm text-stone-500 mb-6 flex-1">{dept.description}</p>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium text-stone-900">{stats.teamMembers}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-stone-500 flex items-center gap-2">
              ...
              <span className="font-medium text-stone-900">{stats.workflows}</span>
            </div>
            ...
            <Button
              ...
              className={cn(
                ...
                'bg-stone-50 hover:bg-stone-100 text-stone-900 border border-stone-200'
              )}
              asChild
            >
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-4xl font-semibold text-stone-900 mb-2">Departments</h1>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12">
          <p className="text-stone-500 mb-2">No departments found</p>
          <p className="text-stone-400 text-sm">Try adjusting your search criteria</p>
        </div>
      )}
    </div>
  );
